...
func CommentRoutes(router *gin.Engine, commentController controller.CommentController, commentService service.CommentService, jwtService service.JWTService) {
	commentRoutes := router.Group("/comment", middleware.Authenticate(jwtService))
	{
		...
		commentRoutes.DELETE("/:commentID", middleware.CommentAuthorization(jwtService, commentService), commentController.UpdateCommentByID)
	}
}
